...
import type { GetResourceResponse, ExpiryData } from 'maplibre-gl';
...
export function getStyle(
  ...
  if (keyring.isGeoloniaStyleCheck(style) && !apiKey) {
    throw new Error(
      '[Geolonia] API key is required to use Geolonia styles.',
    );
  }
 ...
 */
export function parseControlOption(
  value: boolean | string | undefined,
): { enabled: boolean; position: string | undefined } {
...
export function handleRestrictedMode(map: { getContainer: () => HTMLElement; remove: () => void; _geolonia_restricted_mode_handled?: boolean }): void {
