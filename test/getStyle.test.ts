...
describe("getStyle - edge cases", () => {
  ...
  it("should resolve English variant of logical name", () => {
    expect(
      getStyle("geolonia/basic-v2", { lang: "en", apiKey: "test-key" }),
    ).toBe("https://cdn.geolonia.com/style/geolonia/basic-v2/en.json");
  });
  ...
  it("should treat null-ish style as default", () => {
    expect(getStyle("", { lang: "ja", apiKey: "test-key" })).toContain(
      "basic-v2/ja.json",
    );
  });
  ...
  it("should throw for logical name without API key", () => {
    keyring.reset();
    expect(() => getStyle("geolonia/gsi", { lang: "ja", apiKey: "" })).toThrow(
      "API key is required",
    );
  });
  ...
  it("should throw for empty style without API key", () => {
    keyring.reset();
    expect(() => getStyle("", { lang: "ja", apiKey: "" })).toThrow(
      "API key is required",
    );
  });
