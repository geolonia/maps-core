...
import { waitForMapLoad, readCanvasPixelsUpperHalf, collectConsoleErrors } from './helper';
...
test.describe('GeoloniaMap basic rendering', () => {
  ...
  test('should store map instance on container.geoloniaMap', async ({ page }) => {
    await waitForMapLoad(page);
    const hasGeoloniaMap = await page.evaluate(() => {
      const container = document.querySelector('#map') as HTMLElement & { geoloniaMap?: unknown };
      return !!(container && container.geoloniaMap);
    });
    expect(hasGeoloniaMap).toBe(true);
  });

  test('should render tile content in upper half (non-white pixels)', async ({ page }) => {
    await waitForMapLoad(page);
    const hasContent = await readCanvasPixelsUpperHalf(page);
    expect(hasContent).toBe(true);
  });

  test('should not produce application-level console errors', async ({ page }) => {
